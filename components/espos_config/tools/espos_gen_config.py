...
def build_c(namespaces, schema_text, schema_etag, keys_header_name):
    L = []
    L.append("/* Generated by espos_gen_config.py (espos_config/tools) — DO NOT EDIT. */")
    ...
    if namespaces:
        ...
        # A consumer of espos_config that registers no descriptor of its own
        # (a firmware using only the core namespaces from a registry copy, a
        # host test of another component). C11 has no empty array, so the
        # table gets one never-read sentinel; the count is what the runtime
...
def generate(descriptor_paths, schema_out, c_out, h_out):
    """No descriptors is a valid input: the tables come out empty and the schema
    has no properties, so a project that only consumes espos_config still
    links. It used to be an error, which broke exactly those projects."""
    namespaces = []
    seen = {}
...
def main(argv=None):
    ...
    if ns:
        ...
        print("espos_gen_config: no config descriptors registered; empty tables and schema")
    return 0
